package com.devjethava.chplapp.view.base
...
open class BaseFragment<T : ViewDataBinding>(@LayoutRes private val layoutResId: Int) : Fragment() {
    ...
    open fun clearOnDestroyView(isBindingClear: Boolean = true) {
        if (isBindingClear) _binding = null
    }
    ...
    fun <T> Single<T>.bindLifeCycle(owner: LifecycleOwner): SingleSubscribeProxy<T> = this.`as`(
        ...
        if (error is HttpException) {
            if (error.code() == 401) {
                ...
                if (serverMessage == null) {
                    if (message != null) requireActivity().toast(message)
                } else {
